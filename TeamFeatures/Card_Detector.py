...
class CardDetector:
    ...
    def update(self, frame: np.ndarray, players_in_frame: Dict, frame_index: int) -> List[Dict]:
        ...
        for det in detections:
            ...
            # FOR JSON SAFETY: Explicitly cast to native python int
            player_id_int = int(player_id)

            player_stats = self.player_card_counts[player_id_int]
            cap = self.max_cards_per_player[card_type]
            if player_stats[card_type] >= cap:
                continue

            event_key = (player_id_int, card_type)
            last_counted = self._last_counted_event_frame.get(event_key, -10**9)
            if int(frame_index) - last_counted < self.event_cooldown_frames:
                continue

            player_stats[card_type] += 1
            self._last_counted_event_frame[event_key] = int(frame_index)

            raw_team_id = players_in_frame.get(player_id, {}).get("team")
            team_id_int = int(raw_team_id) if raw_team_id is not None else None

            if team_id_int in (1, 2):
                player_stats["team"] = team_id_int
                self.team_card_totals[team_id_int][card_type] += 1
    ...
    def get_summary(self) -> Dict:
        ...
        for player_id, stats in self.player_card_counts.items():
            ...
            raw_team = stats.get("team")
            per_player[int(player_id)] = {
                "team": int(raw_team) if raw_team is not None else None,
                "red": int(stats["red"]),
                "yellow": int(stats["yellow"]),
            }

        # Convert team_totals int64 values to regular Python int
        team_totals_converted = {
            int(team_id): {card_type: int(count) for card_type, count in card_dict.items()}
            for team_id, card_dict in self.team_card_totals.items()
        }

        return {
            "team_totals": team_totals_converted,
